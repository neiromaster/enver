...
// tombstoneRow renders one unset tombstone: the commented-out assignment naming
// the profile that unset the key. parseTombstone recognizes exactly this shape,
// so a rendered document re-imports without the rows landing as comments on the
// next key.
func tombstoneRow(key, by string) string {
	return fmt.Sprintf("# %s=  # unset by %q\n", key, by)
}

// parseTombstone reports whether line is a tombstone row Format emitted and
// returns the key it fences. The match is exact — valid key, `=`, two spaces,
// `# unset by `, quoted profile — so ordinary comments never qualify.
func parseTombstone(line string) (string, bool) {
	...
	const attribution = `  # unset by "`
	if len(rest) < len(attribution)+1 ||
		!strings.HasPrefix(rest, attribution) ||
...
func Format(env map[string]string, comments map[string]string, unsets map[string]string, opts Options) []byte {
	var b strings.Builder
	if opts.Header {
		fmt.Fprintf(&b, "# Generated by enver from profile %q\n", opts.Profile)
		fmt.Fprintf(&b, "# Chain: %s. Values decrypted — do not commit this file.\n\n",
			strings.Join(opts.Chain, " → "))
	}
	for _, k := range envname.SortedUnion(env, unsets) {
		v, live := env[k]
		if !live {
			b.WriteString(tombstoneRow(k, unsets[k]))
			continue
		}
		b.WriteString(formatComment(comments[k]))
		if needsQuote(v) {
			v = formatExpandable(v)
		}
		fmt.Fprintf(&b, "%s=%s\n", k, v)
	}
